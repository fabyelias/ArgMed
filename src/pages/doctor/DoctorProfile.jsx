            ...
            <div className="bg-slate-900/80 border border-cyan-500/30 rounded-2xl p-6">
                <div className="flex flex-col items-center mb-6">
                    <div className="relative w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-cyan-500 mb-4 cursor-pointer group" onClick={() => editing && fileInputRef.current?.click()}>
                        {user?.photo_url ? <img src={user.photo_url} className="w-full h-full object-cover" alt="Doctor Profile" /> : <Stethoscope className="w-full h-full p-6 text-gray-500" />}
                ...
                {editing ? (
                     <div className="space-y-4">
                        ...
                        <input id="doctor-photo" name="doctor-photo" type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
                     </div>
                ) : (
                    <div className="space-y-4">
